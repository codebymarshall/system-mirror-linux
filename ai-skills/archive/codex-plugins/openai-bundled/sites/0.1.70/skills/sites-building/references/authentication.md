...
- For routes that should only be visible after ChatGPT sign-in, use the starter's dispatch-owned SIWC helpers described in the Starter capabilities section of `environment.md` instead of adding app-owned auth.
...
2. For dispatch-owned SIWC routes:
   ...
   - for new sites, follow the Starter capabilities section in `environment.md` to obtain the auth helpers, then import them from `app/chatgpt-auth.ts`.
