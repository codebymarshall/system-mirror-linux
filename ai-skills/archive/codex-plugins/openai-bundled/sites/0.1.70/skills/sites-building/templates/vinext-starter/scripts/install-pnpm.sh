...
if [[ -n "${SITES_PNPM_BIN:-}" && -f "${SITES_PNPM_BIN}" && -r "${SITES_PNPM_BIN}" ]]; then
  ...
  echo "This pnpm project requires pnpm 11.19.0 or Corepack." >&2
  exit 69
fi
...
if [[ "${pnpm_version}" != "11.19.0" ]]; then
  echo "This project requires pnpm 11.19.0 and its v11 store format." >&2
  exit 69
fi
...
try {
  ...
  process.exit(seed.version === 1 && seed.pnpm_version === "11.19.0" &&
